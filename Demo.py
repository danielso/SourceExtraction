...
def GetDefaultParams():
    ...
    'YairDendrites','FISSEQ_MIT','Baylor_multi_Z','Vaughan','Nitzan','Darcy_SEP2017']
    ...
    if data_name=='Hillman' or data_name=='HillmanSmall':
        ...
        mbs=[1] # temporal downsampling of data in intial phase of NMF
        ds=1 # spatial downsampling of data in intial phase of NMF. Ccan be an integer or a list of the size of spatial dimensions
        TargetAreaRatio=[0.001,0.05] # target sparsity range for spatial components
        repeats=60# how many repeations to run NMF algorithm
        iters0=[50] # number of intial NMF iterations, in which we downsample data and add components
        iters=50 # number of main NMF iterations, in which we fine tune the components on the full data
        lam1_s=1# l1 regularization parameter initialization (for increased sparsity). If zero, we have no l1 sparsity penalty
        updateLambdaIntervals=2 # update sparsity parameter every updateLambdaIntervals iterations
        addComponentsIntervals=1 # in initial NMF phase, add new component every updateLambdaIntervals*addComponentsIntervals iterations
        updateRhoIntervals=1 # in main NMF phase, update sparsity learning speed (Rho) every updateLambdaIntervals*updateRhoIntervals iterations
        Background_num=1 #number of background components - one of which at every repetion
        ...
        Background_num=1 #number of background components - one of which at every repetion
        bkg_per=0.1 # intialize of background shape at this percentile (over time) of video
        sig=(200,200,200) # estiamte size of neuron - bounding box is 3 times this size. If larger then data, we have no bounding box.
        MergeThreshold_activity=0.85#merge components if activity is correlated above the this threshold (and sufficiently close)
        MergeThreshold_shapes=0.99 #merge components if activity is correlated above the this threshold (and sufficiently close)        
        ...
        mbs=[1] # temporal downsampling of data in intial phase of NMF
        ds=1 # spatial downsampling of data in intial phase of NMF. Ccan be an integer or a list of the size of spatial dimensions
        TargetAreaRatio=[0.01,0.15] # target sparsity range for spatial components
        repeats=1 # how many repeations to run NMF algorithm
        iters0=[30] # number of intial NMF iterations, in which we downsample data and add components
        iters=50 # number of main NMF iterations, in which we fine tune the components on the full data
        lam1_s=0.00001# l1 regularization parameter initialization (for increased sparsity). If zero, we have no l1 sparsity penalty
        updateLambdaIntervals=2 # update sparsity parameter every updateLambdaIntervals iterations
        addComponentsIntervals=1 # in initial NMF phase, add new component every updateLambdaIntervals*addComponentsIntervals iterations
        updateRhoIntervals=1 # in main NMF phase, update sparsity learning speed (Rho) every updateLambdaIntervals*updateRhoIntervals iterations
        Background_num=1 #number of background components - one of which at every repetion
        bkg_per=20 # intialize of background shape at this percentile (over time) of video
        sig=(600,600) # estiamte size of neuron - bounding box is 3 times this size. If larger then data, we have no bounding box.
        MergeThreshold_activity=0.85 #merge components if activity is correlated above the this threshold (and sufficiently close)
        MergeThreshold_shapes=0.99 #merge components if activity is correlated above the this threshold (and sufficiently close)        
        ...
        mbs=[1]
        ds=1
        TargetAreaRatio=[0.005,0.1]
        repeats=1
        iters0=[10]
        iters=10        
        addComponentsIntervals=100 # in initial NMF phase, add new component every updateLambdaIntervals*addComponentsIntervals iterations
        updateLambdaIntervals=2
        updateRhoIntervals=1
        lam1_s=0.001
        Background_num=1 #number of background components
        ...
        mbs=[1] # temporal downsampling of data in intial phase of NMF
        ds=1 # spatial downsampling of data in intial phase of NMF. Ccan be an integer or a list of the size of spatial dimensions
        TargetAreaRatio=[0.01,0.12] # target sparsity range for spatial components
        repeats=10 # how many repeations to run NMF algorithm
        iters0=[30] # number of intial NMF iterations, in which we downsample data and add components
        iters=100 # number of main NMF iterations, in which we fine tune the components on the full data
        lam1_s=1# l1 regularization parameter initialization (for increased sparsity). If zero, we have no l1 sparsity penalty
        updateLambdaIntervals=2 # update sparsity parameter every updateLambdaIntervals iterations
